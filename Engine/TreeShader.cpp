...
bool TreeShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Engine/TreeVS.hlsl", L"../Engine/TreePS.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TreeShader::Render(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool TreeShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "TreeVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "TreePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
		&pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		MessageBox(hwnd, L"fuckthisshit", L"Missing Shader File", MB_OK);
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
		{
...
void TreeShader::ShutdownShader()
{
...
void TreeShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool TreeShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
void TreeShader::RenderShader(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount)
{
